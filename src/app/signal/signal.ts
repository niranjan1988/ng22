...
export class Signal {
  ...
  protected readonly untrackedDescription = computed(
    () => this.title() + ' - ' + this.subtitle() + ' - ' + untracked(this.description),
  );
  ...
  protected readonly selectedPage = linkedSignal({
    ...
    computation: (newitems, selected) =>
      newitems.find((item) => item === selected?.value) || newitems[0],
  });
